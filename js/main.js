...
function displayChat(content) {
	...
	for (var i = 0; i < 2; i++) {
		...
		var sentStickersCount = words.sentStickersCount;
		...
		for (var j = 0; j < max; j++) {
			mostUsedHTML =
				mostUsedHTML +
				"<p>" +
				escapeHTML(wordsByUsage[i][j][0].substring(1)) +
				" - " +
				Math.round((wordsByUsage[i][j][1] / wordsPerMessage[i][1]) * 1000) / 10 +
				"%</p>";
		}
	...
	filterBtn =
		"<div class='btn-group' id='filterBtn'>" +
	...
	div.className = "mb-0";
	div.innerHTML =
		"<p id='collapseBtnId'><b> Most used words:</b></p>" +
		"<div class='pb-1 mb-1'>" +
		filterBtn +
		"</div>";
	// add to html
	...
	document
		.getElementById("collapseBtnId")
		.insertBefore(collapseBtn, document.getElementById("collapseBtnId").firstChild);
	...
	var wordPDA = Math.round(
		(wordsPerMessage[0][1] + wordsPerMessage[1][1]) / formatedStruct[0].date.length
	);
	...
	div.className = "mb-0";
	div.innerHTML =
		"<p> You guys write an average of " +
		messPDA +
		" messages per day. That's " +
		wordPDA +
		" words per day! </p>" +
		"<p>" +
		n0name +
...
function filterUsers(structArray) {
	...
	// regex
	var reTime = new RegExp(
		"((\\d{1,2})(:|.)(\\d{1,2})((:|.)(\\d{1,2}))?)(\\s(a|p)?m|\\s(A|P)?M|\\s(a|p)?\\.\\sm\\.)?"
	);
...
function addMissingDays(data) {
	...
	for (i = 0; i < data.length; i++) {
		...
		var dates = getDates(
			new Date(data[i].date[0]),
			new Date(data[i].date[data[i].date.length - 1])
		);
...
function getMostUsedWords(messages) {
	...
	return {
		...
		sentStickersCount: sentStickersCount
	};
}
	...
	for (var i = 0; i < 2; i++) {
		...
		for (var j = 0; j < max; j++) {
			mostUsedHTML =
				mostUsedHTML +
				"<p>" +
				filteredWords[i][j][0].substring(1) +
				" - " +
				Math.round((filteredWords[i][j][1] / totalWords) * 1000) / 10 +
				"%</p>";
		}
...
function escapeHTML(text) {
	...
	var getReplacement = function(key) {
		return map[key];
	}; //The function's arg is the result of each match from the replace function
...
function createDayRadar(content) {
	...
	new Chart(document.getElementById("dayRadar"), {
		type: "radar",
		data: {
...
function createChonologicalGraph(content) {
	...
	// create chart
	new Chart(document.getElementById("chronologicalGraph").getContext("2d"), {
		type: "line",
		data: {
